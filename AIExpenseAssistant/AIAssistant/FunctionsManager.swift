            ...
            if let toolCall = message.tool_calls?.first,
               ...
               let argumentData = toolCall.function.arguments.data(using: .utf8) {
                ...
                case .addExpenseLog:
                    ...
                    return .init(text: "비용 목록에 추가하기 전에 확인 버튼을 선택해 주시기 바랍니다", type: .addExpenseLog(.init(log: log, messageID: messageID, userConfirmation: .pending, confirmationCallback: addLogConfirmationCallback)))
                ...
                case .listExpenses:
                    ...
                    if listExpenseArgs.isDateFilterExists {
                        if logs.isEmpty {
                            text = "현재 날짜에 대한 비용 내역이 없습니다"
                        } else {
                            text = "다음은 귀하의 지출 목록과 총 합계입니다: \(Utils.numberFormatter.string(from: NSNumber(value: logs.reduce(0, { $0 + $1.amount }))) ?? "")"
                        }
                    } else {
                        if logs.isEmpty {
                            text = "현재 최근 지출 내역이 없습니다."
                        } else {
                            text = "다음은 최근 \(logs.count) 개의 지출 내역 목록과 총 합계입니다: \(Utils.numberFormatter.string(from: NSNumber(value: logs.reduce(0, { $0 + $1.amount }))) ?? "")"
                        }
                    }
